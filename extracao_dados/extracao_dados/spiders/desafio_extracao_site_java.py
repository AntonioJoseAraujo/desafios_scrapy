'''
DESAFIO: 
# Navege até o site https://f1races.netlify.app e extraia e salve em um arquivo csv os seguintes dados
# Varra todos os 
* nomes de cada grand prix
...
class RacesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for corrida in response_webdriver.xpath("//div[@class='sc-bZQynM llbHfj']"):
            yield {
                'Nome Grand Prix': corrida.xpath("./div[1]/text()").get(),
                'Local': corrida.xpath("./div[2]/text()").get(),
                'Nome Piloto': corrida.xpath(".//a/text()").get(),
                'Tempo da Corrida': corrida.xpath("./div[4]/text()").get(),
            }
        driver.close()
